...
| - alpha = 1, it is an L1 penalty -> Lasso / 변수 축소 - alpha = 0 </br> the penalty is an L2 penalty -> Ridge 가중치 축소 </br> 0 < alpha < 1, elastic net / loss function에 regularization term이 coef의 절대값 항(Lasso)과 coef의 제곱항(Ridge)이 함께 사용   
</br>     - 상관성이 높은 변수 사이에서 Lasso를 사용하면 그 중 하나를 랜덤으로 선택하여 계수 축소하기 때문에, 실제로 가장 중요한 변수가 선택되지 않을 리스크 있음
</br>     - Elastic net의 경우, 상관성 높은 다수의 변수들을 모두 선택하거나 제거 -> group effect 유도
</br>     - 결론적으로 변수 선택 기능도 있음 |
